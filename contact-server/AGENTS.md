...
- `main.go` — entire service. Endpoints: `GET /contact/health` (health
  check) and `POST /contact` (submit form). In-process rate limiter,
  Turnstile CAPTCHA verification, input sanitization, HTML escaping, and
  email header-injection prevention all live here.
...
- `nginx/api.nukehub.org.conf` — top-level nginx vhost that:
  ...
  - Serves `/contact` (proxy_pass to `127.0.0.1:3000`) and applies the
