...
function GlowingOrbits() {
  const shouldReduceMotion = useReducedMotion();
  const orbs = [
    { size: 80, distance: 90, duration: 22, delay: 0, opacity: 0.45 },
    { size: 60, distance: 130, duration: 30, delay: 2, opacity: 0.35 },
    { size: 70, distance: 160, duration: 38, delay: 5, opacity: 0.3 },
  ];
    ...
    <div className="absolute inset-0 pointer-events-none">
      {orbs.map((orb, i) => (
        <motion.div
          ...
          style={{
            width: orb.distance * 2 + orb.size,
            height: orb.distance * 2 + orb.size,
            marginLeft: -(orb.distance * 2 + orb.size) / 2,
            marginTop: -(orb.distance * 2 + orb.size) / 2,
          }}
        ...
        >
          <div
            ...
            style={{
              ...
              background: `radial-gradient(circle, color-mix(in oklch, var(--primary) ${Math.round(orb.opacity * 100)}%, transparent) 0%, transparent 70%)`,
              filter: 'blur(24px)',
            }}
          />
        </motion.div>
...
export function AboutCTA({ title, description, primaryCta, secondaryCta }: Props) {
  const shouldReduceMotion = useReducedMotion();
