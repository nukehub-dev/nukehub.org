...
import { MissionCanvas } from '@components/three/MissionCanvas';
...
function PillarCard({ pillar, index }: { pillar: MissionData['pillars'][0]; index: number }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0, x: entrance.x, y: entrance.y, scale: entrance.scale }}
    ...
    >
      <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card p-7 shadow-lg shadow-black/[0.06] transition-all duration-500 hover:-translate-y-1.5 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/[0.08] dark:border-border/25 dark:bg-card/40 dark:shadow-sm dark:hover:border-primary/20 dark:hover:bg-card/50 dark:hover:shadow-2xl dark:hover:shadow-primary/[0.04] sm:p-9">
        ...
        <div className="absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-primary/30 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100 dark:via-primary/20" />

        {/* Giant step watermark */}
        <span className="pointer-events-none absolute right-2 top-1 select-none text-[5.5rem] font-extrabold leading-none text-primary/20 transition-colors duration-500 group-hover:text-primary/30 dark:text-foreground/[0.025] dark:group-hover:text-primary/[0.05] sm:text-[7rem]">
          {step}
        </span>
