...
export function GlassContextMenu({ children, scopeRef, title }: GlassContextMenuProps) {
...
export function ContextMenuItem({
  ...
  if (href && !disabled) {
    return (
      <a href={href} target={target} rel={rel} className={baseClass} onClick={handleClick}>
        {content}
      </a>
    );
  }
  ...
  return (
    <button type="button" className={baseClass} onClick={handleClick} disabled={disabled} {...props}>
      {content}
    </button>
  );
}
...
export function ContextMenuSub({ label, icon: Icon, children }: ContextMenuSubProps) {
        ...
        {Icon && (
          <span className="flex h-5 w-5 shrink-0 items-center justify-center">
            <Icon
              className={cn(
                'h-4 w-4 transition-colors duration-150',
                open ? 'text-foreground' : 'text-muted-foreground/70 group-hover:text-foreground'
              )}
            />
          </span>
        )}
        ...
        createPortal(
          <div
            ...
            style={{
              ...
              top: subPos.y,
              opacity: ready ? 1 : 0,
              animation: ready ? 'cmFadeIn 0.15s cubic-bezier(0.16,1,0.3,1)' : 'none',
              boxShadow:
...
/* ───────────────────────────────────────────────────────────
   ContextMenuLabel
   ─────────────────────────────────────────────────────────── */
export function ContextMenuLabel({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <div
      className={cn(
