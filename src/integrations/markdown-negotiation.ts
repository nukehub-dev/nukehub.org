...
function extractMeta(html: string): { title: string | null; description: string | null } {
  ...
  if (titleMatch) {
    title = titleMatch[1].trim().replace(/\s*\|\s*NukeHub\s*$/i, "").trim() || null;
  }
...
async function walk(dir: string, root: string, turndown: typeof TurndownService.prototype): Promise<void> {
